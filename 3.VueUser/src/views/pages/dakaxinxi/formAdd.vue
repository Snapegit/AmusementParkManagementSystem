	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 18%","margin":"20px auto 60px","background":"#fff","width":"100%","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="打卡类型" prop="dakaleixing">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.dakaleixing?true:false"
							v-model="form.dakaleixing" 
							placeholder="请选择打卡类型"
							style="width:100%;"
							>
							<el-option v-for="(item,index) in dakaleixingLists" :label="item"
								:value="item"
								>
							</el-option>
						</el-select>
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='dakaleixing'){
					form.value.dakaleixing = row[x];
					disabledForm.value.dakaleixing = true;
					continue;
				}
				if(x=='dakashijian'){
		...
		context?.$http({
			...
			if(json.hasOwnProperty('yuangonggonghao') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
				form.value.yuangonggonghao = json.yuangonggonghao
				disabledForm.value.yuangonggonghao = true;
			}
			if(json.hasOwnProperty('yuangongxingming') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
