	...
	<div class="app-contain" :style='{"width":"100%","padding":"0 18%","margin":"0","overflow":"hidden","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('menpiaogoumai','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="购买时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/menpiaogoumaiAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('menpiaogoumaiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
