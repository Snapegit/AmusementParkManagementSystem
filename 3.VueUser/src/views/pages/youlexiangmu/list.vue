<template>
	<div class="app-contain" :style='{"width":"100%","padding":"0 18%","margin":"0","overflow":"hidden","background":"#fff"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('youlexiangmu','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.tupian && item.tupian.substr(0,4)=='http'" @click.stop="preViewClick(item.tupian)">
							<el-image class="data_img" :src="item.tupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.tupian.split(',')[0])">
							<el-image class="data_img" :src="item.tupian?$config.url + item.tupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title">项目名称：{{item.xiangmumingcheng}}</div>
							<div class="data_title">{{item.kaifangshijian}}</div>
							<div class="data_title">价格：{{item.jiage}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/youlexiangmuAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.xiangmufenlei = categoryList.value[categoryIndex.value]
		}
		...
		if (searchQuery.value.jiagestart&&searchQuery.value.jiagestart!='') {
			params.jiagestart = searchQuery.value.jiagestart
		}
		if (searchQuery.value.jiageend&&searchQuery.value.jiageend!='') {
	...
	const detailClick = (id) => {
		router.push('youlexiangmuDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
